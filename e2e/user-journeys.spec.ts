...
test.describe('Deep End-to-End Journeys', () => {
  ...
  // Open detail and verify both files
  ...
  // Edit: rename and upload a third file
  await page.getByRole('button', { name: /Изменить/i }).click();
  ...
  // Verify all 3 files on detail page after save
  ...
  // Profile: check storage section is visible
  await page.goto('/profile');
  await expect(page.getByRole('heading', { name: 'Профиль' })).toBeVisible();
  await expect(page.getByText(/Использовано|Хранилище|Место/i)).toBeVisible();

  // Return to dashboard, search for edited note, and delete it
  await page.goto('/dashboard');
  ...
  await openTodoDetail(page, updatedTitle);
  await page.getByRole('button', { name: /Удалить/i }).click();
  const deleteDialog = page.getByRole('alertdialog', { name: 'Удалить задачу?' });
